...
syn_flood_store = Chroma(
    persist_directory="./chroma_db",
    embedding_function=embedding_model,
    collection_name="syn_flood_alerts"
)
...
# === Prompt Template ===
syn_flood_prompt = PromptTemplate(
    # Define input variable names to be dynamically filled in later
    input_variables=["syn_flood_alerts", "anomaly_csv", "heuristic_context"],

    # Template string containing detailed instructions for the language model
    template="""
You are a cybersecurity analyst AI trained to detect, interpret, and explain TCP SYN flood attacks.

You are given:
- `syn_flood_alerts`: Natural-language summaries of potential SYN flood events.
...
Your goals:
1. Confirm whether each SYN flood detection is consistent with known anomaly patterns.
...
- SYN flood alert: [summary]
  Justified by anomaly: Matched anomaly ID [ID], heuristic [ID] ([desc]), taxonomy [name] ([desc]).
  → Action: [specific action like block IP, investigate source, rate-limit connections, etc.]
...
def generate_rag_analysis(query_text, similarity_threshold=0.3):
    ...
    # Treat the user query itself as the main SYN flood alert content
    syn_flood_contents = [query_text]

    # Assemble the context dictionary to populate the prompt template
    context = {
        "syn_flood_alerts": "\n".join(syn_flood_contents) or "No alerts found.",
    ...
    prompt = syn_flood_prompt.format(**context)
...
def prepare_conn_log_for_llm(file_path):
    ...
    Filters to TCP traffic by default, falls back to all traffic if no TCP found.
    ...
    try:
        ...
        # Filter lines to those with proto field == 'tcp'
        tcp_lines = [line for line in lines if len(line.split('\t')) > 6 and line.split('\t')[6].lower() == 'tcp']
        
        # If no TCP lines, fallback to all lines
        return "\n".join(tcp_lines) if tcp_lines else "\n".join(lines)
...
def generate_rag_analysis_from_log(conn_log_text):
    ...
    context = {
        "syn_flood_alerts": cap_text(conn_log_text, 2500),
...
1️⃣  Interactive Mode (Advanced RAG with thresholds & reranking):
   ...
   Example: "Heavy TCP SYN flood to 192.168.1.100 at midnight"
...
💡 What you can enter:
- A quick note like "Heavy TCP SYN flood from 10.0.0.5 at midnight"
- A suspicious event summary: "Unexpected scan-like surge to web server"
- A curious observation: "Strange high-volume SYN packets from unknown host"
- Metadata filters: "DoS attack on port 80 to 192.168.1.1"
